class Frete {
  ...
  getDimensions(items){
    ...
    boxDim.altura = parseFloat(maxVol.altura);
    boxDim.comprimento = parseFloat(maxVol.comprimento);
    boxDim.largura = parseFloat(maxVol.largura);
    ...
    if(organizedItems.length > 0){
      ...
      while(organizedItems.length > 0){
        ...
        if(freeTopW === boxDim.largura){
          ...
          if(maxVol.largura <= freeTopW){
            freeTopW -= parseFloat(maxVol.largura);
            ...
            lastItem = maxVol;
            boxDim.peso += parseFloat(maxVol.peso);
            boxDim.volItems += parseFloat(maxVol.comprimento) * parseFloat(maxVol.altura) * parseFloat(maxVol.largura);
            this.discount(organizedItems, maxVol);
        ...
        // Caso já exista itens no topo 
          ...
          // Largura do item cabe
          if(parseFloat(maxVol.largura) <= freeTopW){
            freeTopW -= parseFloat(maxVol.largura);
    
            // Caso item extrapole comprimento
            if(parseFloat(maxVol.comprimento) > parseFloat(boxDim.comprimento))
              boxDim.comprimento = parseFloat(maxVol.comprimento);
    
            // Item será considerado como último se sua altura for a maior do topo
            if(parseFloat(maxVol.altura) > parseFloat(lastItem.altura))
              lastItem = maxVol;
          ...
          // Caso a largura do item não caiba, um novo nível é formado
            ...
            boxDim.altura += parseFloat(lastItem.altura);
            freeTopW = parseFloat(boxDim.largura);
          }
      ...
      boxDim.altura += parseFloat(lastItem.altura);
      boxDim.peso += parseFloat(lastItem.peso);
          
    }
    
    boxDim.volItems += parseFloat(lastItem.comprimento) * parseFloat(lastItem.altura) * parseFloat(lastItem.largura);;
